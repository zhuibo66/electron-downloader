...
class aria2cModule {
    ...
    //连接rpc
    connect() {
        this.rpc = new AriaJsonRPC(hostUrl, secret, this.onAriaResponse, this.onAriaError);
    ...
    addUris(uris: string[], options: Options) {
        const requests = uris.map(uri => this.rpc.call('aria2.addUri', [[uri], options]));
...
type Status = 'active' | 'waiting' | 'paused' | 'error' | 'complete' | 'removed' | 'noLocalFile';
