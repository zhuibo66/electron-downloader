...
interface FileObject {
  ...
  fileName: string;
  fileSize: string;
  fullFileSavePath: string;
  fileIcon: string;
  fileStatus?: "noLocalFile" | string;
  downloadUrl: string;
  downloadStatus: "pause" | "start" | "completed" | "downloading" | "error"; //pause start complete
  donwloadSpeed: number;
  downloadProgress: number;
  downloadedBytes: number; //已经下载的字节
  [index: string]: any;
}
...
const App = () => {
  ...
  const [visibleDeleteDownloadTaskModal, setVisibleDeleteDownloadTaskModal] =
    useState<boolean>(false); //显示删除文件的模态窗
  const [deleteLocalFileCheckBox, setDeleteLocalFileCheckBox] =
    useState<boolean>(true); //是否删除本地文件
  const [curDeleteDownloadTaskInfo, setCurDeleteDownloadTaskInfo] =
    useState<any>({}); //当前正在删除的任务文件信息
  ...
  const closeAddDownloadTaskModal = () => {
    ...
    localStorage.setItem(
      "lastFileSavePath",
      form.getFieldValue("fileSavePath")
    );
    form.resetFields();
                ...
                <>
                    ...
                    return (
                      ...
                      >
                        <Form.Item
                          name={[field.name, "key"]}
                          rules={[
                            { required: true, message: "请输入Header key" },
                          ]}
                          style={{
                            width: "100%",
                          }}
                  ...
                  >
                    ...
                    <Button
                      type="primary"
                      danger
                      onClick={() => remove(fields.map((item) => item.name))}
                      icon={<MinusOutlined />}
                    >
                      移动所有新添加项
                    </Button>
  ...
  const onFormFieldsChange = (changedFields, allFields) => {
    ...
    isErrorLength
      ? setValidAllFormFormatStatus(true)
      : setValidAllFormFormatStatus(false);
  };
  ...
  const onDownloadInputBlur = () => {
    ...
    if (downloadUrlValue.length == 0 || downloadUrlValueError.length) {
      ...
      let matchFileName = downloadUrlValue.replace(
        new RegExp("[a-zA-z]+://"),
        ""
      );
      ...
      if (hasForwardSlash == -1) {
        ...
        if (matchFileName) {
          ...
          matchFileName = matchFileName.replace(
            /\\|\/|\:|\*|\?|\"|\<|\>|\||\^/g,
            "_"
          );
        } else {
  ...
  return (
    <div className="App">
      <List
        ...
        renderItem={(item) => (
          <List.Item
            className={`file-list status-${item.status}`}
          ...
          <Row>
            <Col span={24} style={{ textAlign: "right" }}>
              ...
              <Button
                ...
                htmlType="submit"
                style={{ margin: "0 8px" }}
                disabled={validAllFormFormatStatus}
              >
                下载
              </Button>
            </Col>
      ...
      >
        ...
        <p>
          <div className="left">
            <Checkbox
              onChange={(e) => setDeleteLocalFileCheckBox(e.target.checked)}
              checked={deleteLocalFileCheckBox}
            >
              同时删除文件
            </Checkbox>
          </div>
